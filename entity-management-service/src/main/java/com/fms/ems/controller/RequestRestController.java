...
public class RequestRestController {
	...
	@GetMapping("/requests")
	@Transactional
	public ResponseEntity<RMSAllRequestsResponse> getAllRequests() {
		try {
			RMSAllRequestsResponse requests = restTemplate.getForObject(rmsEndpoint + "/requests",
					RMSAllRequestsResponse.class);
			if (requests.isEmpty()) {
	...
	@Transactional
	public ResponseEntity<RMSAllRequestsResponse> getAllRequestsByProjectId(@PathVariable int projectId) {
		try {
			RMSAllRequestsResponse requests = restTemplate.getForObject(rmsEndpoint + "/" + projectId + "/requests",
					RMSAllRequestsResponse.class);
			if (requests.isEmpty()) {
				return new ResponseEntity<>(HttpStatus.NO_CONTENT);
			} else {
