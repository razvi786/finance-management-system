...
public class RoleRestController {
  ...
  public ResponseEntity<Role> createRole(@RequestBody Role role) {
    try {
      if (role.getPermissions() != null) {
        ...
        final List<Permission> permissionsFromDatabase = new ArrayList<>();
        for (final Permission permission : permissions) {
          final Optional<Permission> permissionOptional =
              permissionRepository.findById(permission.getPermissionId());
          if (permissionOptional.isPresent()) {
            permissionsFromDatabase.add(permissionOptional.get());
          } else {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
          }
        }
        role.setPermissions(permissionsFromDatabase);
      }
  ...
  public ResponseEntity<Role> addPermissionToRole(
    ...
    try {
      ...
      Optional<Permission> permissionOpt =
          permissionRepository.findById(Integer.parseInt(permissionId));
  ...
  @Transactional
  public ResponseEntity<Role> deletePermissionFromRole(
      @PathVariable String roleId, @PathVariable String permissionId) {
    try {
      Optional<Role> roleOpt = roleRepository.findById(Integer.parseInt(roleId));
      if (roleOpt.isPresent()) {
        ...
        Optional<Permission> rolePermissionToBeDeleted =
            rolePermissions.stream()
                .filter(
                    permission -> permission.getPermissionId() == Integer.parseInt(permissionId))
                .findAny();
